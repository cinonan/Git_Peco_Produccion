...
namespace AzureSearch.DataApp.Publico.Processes
{
    public class PublicoUploadProcess
    {
        ...
        #region "Constructors"
        public PublicoUploadProcess(IConfiguration configuration)
        {

            _configuration = configuration;

            targetSearchServiceName = _configuration["AzureSearch:Load:ServiceName"];
            targetAdminKey = _configuration["AzureSearch:Load:AdminKey"];
            targetIndexName = _configuration["AzureSearch:Load:IndexName"];
            targetAgreementIndexName = _configuration["AzureSearch:Load:AgreementIndexName"];
            targetCatalogueIndexName = _configuration["AzureSearch:Load:CatalogueIndexName"];
            targetCategoryIndexName = _configuration["AzureSearch:Load:CategoryIndexName"];
            backupDirectory = _configuration["AzureSearch:Load:Directory"];
        ...
        private async Task ProcessIndexIncrementallyAsync<T>(SearchClient searchClient, List<T> newDocuments, Func<T, string> getId, string indexFriendlyName) where T : class
        {
            ...
            var existingHashes = await GetCurrentHashesAsync(searchClient, targetSearchServiceName, targetAdminKey);
        ...
        /// <summary>
        /// Obtiene un diccionario de todos los documentos existentes en un índice (ID -> ContentHash)
        /// utilizando directamente la API REST para garantizar una paginación robusta.
        /// </summary>
        /// <remarks>
        /// Se optó por la API REST, usando exclusivamente el método GET, para superar las inconsistencias
        /// observadas en la paginación del SDK y las respuestas del endpoint de búsqueda POST.
        /// Este enfoque, que codifica la consulta inicial en la URL, ha demostrado ser el más
        /// fiable para forzar a la API a devolver el `@odata.nextLink` necesario para una paginación completa.
        /// </remarks>
        public static async Task<int> GetDocumentCountAsync(SearchClient searchClient, string serviceName, string adminKey, CancellationToken cancellationToken = default)
        {
            var requestUri = $"https://{serviceName}.search.windows.net/indexes/{searchClient.IndexName}/docs/$count?api-version=2020-06-30";

            using (var requestMessage = new HttpRequestMessage(HttpMethod.Get, requestUri))
            {
                requestMessage.Headers.Add("api-key", adminKey);
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    {
                        return 0; // El índice no existe, por lo que no tiene documentos.
                    }
                    var errorContent = await response.Content.ReadAsStringAsync();
                    throw new HttpRequestException($"Error al obtener el conteo de documentos: {response.StatusCode} - {errorContent}");
                }
        ...
        {
            // Obtener el conteo total de documentos para inicializar el diccionario con la capacidad óptima.
            var documentCount = await GetDocumentCountAsync(searchClient, serviceName, adminKey, cancellationToken);
            var hashes = new Dictionary<string, string>(capacity: documentCount > 0 ? documentCount : 32); // Usar una capacidad mínima por si acaso
            ...
            while (moreResults)
            {
                // Construye la URI de la solicitud
                var requestUri = $"https://{serviceName}.search.windows.net/indexes/{searchClient.IndexName}/docs" +
                                 $"?api-version=2020-06-30" +
                                 $"&$select=Id,ContentHash" +
                                 $"&$top=1000" + // Límite de tamaño de página de Azure
                                 $"&$orderby=Id asc";

                // Si no es la primera página, añade un filtro para obtener el siguiente rango
                if (lastId != null)
                {
                    requestUri += $"&$filter=Id gt '{lastId}'";
                }
                ...
                using (var requestMessage = new HttpRequestMessage(HttpMethod.Get, requestUri))
                {
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                        ...
                        var errorContent = await response.Content.ReadAsStringAsync();
                        throw new HttpRequestException($"Error al consultar la API de Azure Search: {response.StatusCode} - {errorContent}");
                    }

                    var content = await response.Content.ReadAsStringAsync();
                    var jsonResponse = JsonDocument.Parse(content);
                    var documents = jsonResponse.RootElement.GetProperty("value").EnumerateArray().ToList();

                    if (documents.Any())
                    {
                        foreach (var doc in documents)
                        {
                            if (doc.TryGetProperty("Id", out var idElement) && doc.TryGetProperty("ContentHash", out var hashElement))
                            {
                        ...
                        MessageUtil.Write(true, $"Azure Search: {searchClient.IndexName} -> {hashes.Count} registros recuperados.");
                        // Obtiene el ID del último documento en el lote actual
                        lastId = documents.Last().GetProperty("Id").GetString();
                    }
                    else
                    {
                        // No hay más documentos, salir del bucle
                        moreResults = false;
                    }
                }
            }
        ...
        private async Task CreateProductSheetIndexAsync()
        {
            ...
            try
            {
               ...
               {
                   // Clave primaria
                   new SimpleField(nameof(PublicoProductIndex.Id), SearchFieldDataType.String) { IsKey = true, IsSortable = true, IsFilterable= true},
