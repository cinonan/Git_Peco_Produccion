...
BEGIN
	...
	if @table = 'Agreement' 
		...
		begin
			select top 100 * from [dbo].[TMP_Publico_Product];
		end
	else if @table = 'Department'
		begin
			select top 1000 * from [dbo].[TMP_Publico_ProductoDepartmento];
		end
	else if @table = 'Feature'
		begin
			select top 1000 * from [dbo].[TMP_Publico_Caracteristica];
		end
	else if @table = 'ProductFeature'
		begin
			select top 1000 * from [dbo].[TMP_Publico_ProductFeature];
		end
