...
namespace CEAM.AzureSearch.WebApp.Services
{
    ...
    public class AzureSearchService : IAzureSearchService
    {
        ...
        {
            var agreementServerList = new List<FilterDataModel>();

            if (model.IsNewSearch) agreementServerList = GetServerAgreement(model);
            else
            {
                var agreementText = string.IsNullOrWhiteSpace(model.ServerFilter.Agreement) ? "" : model.ServerFilter.Agreement.Trim();
                agreementServerList = string.IsNullOrWhiteSpace(agreementText) ? null : JsonSerializer.Deserialize<List<FilterDataModel>>(agreementText);
            }

            model.ServerFilter.Agreement = agreementServerList != null ? JsonSerializer.Serialize(agreementServerList) : "";

            if (agreementServerList != null)
            {
                //var agreementCount = model.Results.Facets["Agreement/SearchText"].OrderBy(o => o.Value).Select(x => new { Name = x.Value.ToString(), Count = x.Count.ToString() }).ToList();
                agreementServerList.ForEach(i =>
                {
                    i.Text = i.Value + "S (" + i.Count + ")";
                    i.Items.ForEach(j => { j.Text = j.Value.Substring(j.Value.ToString().IndexOf(StringHelper.Separator) + 1) + " (" + j.Count + ")"; j.IsChecked = ""; });
                });

                var newAgreementServerList = GetServerAgreement(model);
                var agreementClientList = new List<string>();
                if (!string.IsNullOrWhiteSpace(model.ClientFilter.Agreement)) agreementClientList = JsonSerializer.Deserialize<List<string>>(model.ClientFilter.Agreement);

                if (agreementClientList != null && agreementClientList.Any())
                {
                    var newAgreements = new List<FilterItemModel>();
                    newAgreementServerList.ForEach(a =>
                    {
                        a.Items.ForEach(b => { if (agreementClientList.Contains(b.Value)) newAgreements.Add(b); });
                    });

                    if (newAgreements != null && newAgreements.Any())
                    {
                        agreementServerList.ForEach(i =>
                        {
                            i.Text = i.Value + " (0)";
                            i.Count = 0;
                            i.Items.ForEach(j => { j.Text = j.Value.Substring(j.Value.ToString().IndexOf(StringHelper.Separator) + 1) + " (0)"; j.IsChecked = ""; j.Count = 0; });
                        });
                        ...
                        agreementServerList.ForEach(i =>
                        {
                            i.Items.ForEach(j =>
                            {
                                var agreementItem = newAgreements.Where(w => w.Value == j.Value).Select(s => s).FirstOrDefault();

                                if (agreementItem != null)
                                {
                                    j.Text = agreementItem.Text;
                                    j.Count = agreementItem.Count;
                                    j.IsChecked = "checked";
                                }
                            });
                        });
                        ...
                        agreementServerList.ForEach(i =>
                        {
                            i.Text = i.Value + "S (" + i.Items.Select(s => s.Count).Sum().ToString() + ")";
                            i.Count = i.Items.Select(s => s.Count).Sum();
                            i.Items.ForEach(j => { j.Text = j.Value.Substring(j.Value.ToString().IndexOf(StringHelper.Separator) + 1) + " (" + j.Count + ")"; });
                        });
                    }
                }
                ...
                agreementServerList.ForEach(i =>
                {
                    ...
                    {
                        j.Count = 0;
                        j.Text = j.Value.Substring(j.Value.ToString().IndexOf(StringHelper.Separator) + 1) + " (0)";
                    });
                });

                newAgreementServerList.ForEach(nas =>
                {
                    nas.Items.ForEach(nasi =>
                    {
                        agreementServerList.ForEach(asl =>
                        {
                            asl.Items.ForEach(asli =>
                            {
                                if (nasi.Value == asli.Value)
                                {
                                    asli.Count = nasi.Count;
                                    asli.Text = nasi.Value.Substring(nasi.Value.ToString().IndexOf(StringHelper.Separator) + 1) + " (" + nasi.Count + ")";
                                }
                            });
                        });
                    });
                ...
                agreementServerList.ForEach(i =>
                {
                    i.Text = i.Value + "S (" + i.Items.Where(w => w.Count > 0).ToList().Count().ToString() + ")";
                    i.Count = i.Items.Where(w => w.Count > 0).ToList().Count();
                });

                model.AgreementFilter = agreementServerList;
            }
        }
        ...
        {
            // Merge incremental: evitar "zeroing" global y actualizar solo items coincidentes.
            var featureClientList = new List<string>();
            List<FilterDataModel> featureServerList;

            // Leer filtros cliente si existen
            if (!string.IsNullOrWhiteSpace(model?.ClientFilter?.Feature))
            {
                try { featureClientList = JsonSerializer.Deserialize<List<string>>(model.ClientFilter.Feature) ?? new List<string>(); }
                catch { featureClientList = new List<string>(); }

                // Intentar usar la lista serializada como base; si no existe, usar facets actuales
                try
                {
                    featureServerList = string.IsNullOrWhiteSpace(model.ServerFilter.Feature)
                        ? GetServerFeature(model) ?? new List<FilterDataModel>()
                        : JsonSerializer.Deserialize<List<FilterDataModel>>(model.ServerFilter.Feature) ?? GetServerFeature(model) ?? new List<FilterDataModel>();
                }
                catch
                {
                    featureServerList = GetServerFeature(model) ?? new List<FilterDataModel>();
                }
            }
            else
            {
                // Sin filtros cliente, la fuente de la verdad son los facets actuales
                featureServerList = GetServerFeature(model) ?? new List<FilterDataModel>();
            }

            // Obtener la versión "fresca" de facets para hacer merge de counts
            var freshFeatureServerList = GetServerFeature(model) ?? new List<FilterDataModel>();

            // Construir lookup Value -> FilterItemModel desde la lista fresca
            var freshLookup = freshFeatureServerList
                .SelectMany(g => g.Items ?? Enumerable.Empty<FilterItemModel>())
                .Where(i => i != null && !string.IsNullOrWhiteSpace(i.Value))
                .ToDictionary(i => i.Value, i => i, StringComparer.OrdinalIgnoreCase);

            // Para cada grupo/item en la lista base, actualizar SOLO los items encontrados en freshLookup.
            foreach (var group in featureServerList)
            {
                if (group == null) continue;
                group.Items ??= new List<FilterItemModel>();

                foreach (var item in group.Items)
                {
                    if (item == null || string.IsNullOrWhiteSpace(item.Value)) continue;

                    // Si existe información fresca, actualizamos Count (y reescribimos Text más abajo)
                    if (freshLookup.TryGetValue(item.Value, out var fresh))
                    {
                        item.Count = fresh.Count;
                    }

                    // Normalizar texto de visualización (parte después del separador)
                    var display = item.Value ?? "";
                    var idx = display.IndexOf(StringHelper.Separator);
                    if (idx >= 0 && idx + 1 < display.Length) display = display.Substring(idx + 1);
                    item.Text = $"{display} ({item.Count})";

                    // Mantener marcado según cliente
                    item.IsChecked = (featureClientList != null && featureClientList.Contains(item.Value)) ? "checked" : "";
                }

                // Recalcular totales del grupo como suma de los counts actuales
                group.Count = group.Items?.Sum(i => i.Count) ?? 0;
            ...
            // Ordenar grupos por contador descendente como antes
            featureServerList = featureServerList.OrderByDescending(g => g.Count).ToList();

            // Persistir/serializar y asignar al modelo
            model.ServerFilter.Feature = featureServerList != null ? JsonSerializer.Serialize(featureServerList) : "";
            model.FeatureFilter = featureServerList;
        }
        ...
        public async Task<List<FilterItemModel>> SearchByAgreementStatus(SearchDataModel model, string status)
        {
            ...
            ConfigureSearch();
            ConfigureSearchOptions(ref options, searchText);

            options.Filter = "(Agreement/Status eq '" + status + "')";

            options.Facets.Add("Agreement/SearchText,count:100000");
            //options.Facets.Add("Agreement/Name,count:100000");
            options.Select.Add("Agreement/Name");
            options.Select.Add("Agreement/Status");
            options.Select.Add("Agreement/SearchText");

            var canonicalQuery = _normalizer.Normalize(searchText);
            model.Results = await _searchClient.SearchAsync<PublicoProductDocument>(canonicalQuery, options).ConfigureAwait(false);
        ...
        public async Task<List<PublicoProductDocument>> DownLoadAsync(SearchDataModel model)
        {
            ...
            options.Skip = 0; // page * GlobalVariables.ResultsPerPage;
            options.Size = 99999; // GlobalVariables.ResultsPerPage;
            options.IncludeTotalCount = true;

            // Return information on the text, and number, of facets in the data.
            //options.Facets.Add("Agreement/SearchText,count:1000"); //100000
            //options.Facets.Add("Catalogue/Name,count:1000");
            ...
            //options.Select.Add("FeatureByTypeList");
            options.Select.Add("FeatureTypeList");

            // For efficiency, the search call should be asynchronous, so use SearchAsync rather than Search.
            var canonicalQuery = _normalizer.Normalize(searchText);
            var response = await _searchClient.SearchAsync<PublicoProductDocument>(canonicalQuery, options).ConfigureAwait(false);
            var values = response.Value.GetResultsAsync();

            var list = new List<PublicoProductDocument>();
            var items = response.Value.GetResults();
            ...
            //var datos = new List<SearchResult<PublicoProductDocument>>();
            //var count = values.TotalCount;

            //while (datos.Count < count ) {
            //    foreach (SearchResult<PublicoProductDocument> item in items)
            //    {
            //        //list.Add(item.Document);
            //        var thread = new Thread(() => datos.Add(item));
            //        thread.Start();
            //    }
            //}
            //list = datos.Select(s => s.Document).ToList();
        ...
        private SearchOptions BuildBaseOptions(string filters, int page, bool isHybrid = false)
        {
            var options = new SearchOptions
            {
                Filter = filters,
                Skip = page * GlobalVariables.ResultsPerPage, // Usa la paginación para el resultado final
        ...
        private async Task<SearchResults<PublicoProductDocument>> RunKeywordSearchAsync(string query, string filters, int page)
        {
            var options = BuildBaseOptions(filters, page);
            var resp = await _searchClient.SearchAsync<PublicoProductDocument>(query, options).ConfigureAwait(false);
            return resp.Value; // ← devolvemos SearchResults<T>
        }
        ...
        public async Task<SearchDataModel> RunQueryAsync(SearchDataModel model)
        {
            ...
            // ===== Validación previa antes de decidir estrategia =====
            // Si el texto está vacío o excede un tamaño razonable (ej. 400 caracteres), no intentamos híbrida.
            ...
            try
            {
                // 1) normaliza a formato canónico para BM25 y vector
                var canonicalQuery = _normalizer.Normalize(searchText);

                // 2) usa la canónica en ambos motores
                var queryForBoth = canonicalQuery;


                if ((wordCount > 0) & (page == 0))
                {
                    FileLog.GuardarArchivo(nombreArchivo, prefix + searchText);
                }

                // Corte preventivo: query vacío o demasiado largo → solo keyword
                if (string.IsNullOrWhiteSpace(queryForBoth) || queryForBoth.Length > 200)
                {
                    model.Results = await RunKeywordSearchAsync(queryForBoth, filters, page);
                }
                else if (wordCount == 1)
                {
                    // KEYWORD
                ...
                {
                    // HÍBRIDA 
                    model.Results = await RunHybridSearchAsync(queryForBoth, filters, page);
                }
            }
            catch (Exception e)
            {
            ...
            // ===== Paginación =====
            //Esta sección hay que hacerla que soporte el null de Results, de aqui hacia abajo falla
            var totalCount = (int)(model.Results?.TotalCount ?? 0L);
            model.Pagination.PageCount = (totalCount + GlobalVariables.ResultsPerPage - 1) / GlobalVariables.ResultsPerPage;
        ...
        private async Task<SearchResults<PublicoProductDocument>> RunHybridSearchAsync(
            string query,
            string filters,
            int page,
            double vectorSimilarityThreshold = 0.85
        )
        {
            // --- 1) Opciones base (incluye paginación final y facets) ---
            var searchOptions = BuildBaseOptions(filters, page, isHybrid: true);
            ...
            catch (Exception ex)
            {
                ...
                return await RunKeywordSearchAsync(query, filters, page);
            }
        ...
        public async Task<List<FilterDataModel>> RunQueryForOtherFeaturesAsync(SearchDataModel model)
        {
            ...
            // If Select is empty, all values will be returned, which can be inefficient.
            //options.Select.Add("FeatureByTypeList");
            options.Select.Add("Features");

            var canonicalQuery = _normalizer.Normalize(searchText);
            model.Results = await _searchClient.SearchAsync<PublicoProductDocument>(canonicalQuery, options).ConfigureAwait(false);
            model.SearchTextPrevious = canonicalQuery;
            ...
            foreach (var result in results) features.AddRange(result.Document.Features);
