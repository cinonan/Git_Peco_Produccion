        ...
        private List<object[]> GetBody(List<PublicoProductDocument> list)
        {
            ...
            if (list != null && list.Any())
            {
                list.ForEach(x =>
                {
                    ...
                    parte = x.FeatureTypeList.Where(a => a.Text.ToUpper() == "NRO_PARTE" || a.Text.ToUpper() == "NRO PARTE" ||
                                                    a.Text.ToUpper() == "CODIGO_DE_IDENTIFICACION_UNICO" || a.Text.ToUpper() == "CODIGO DE IDENTIFICACION UNICO").Select(a => a.Values.FirstOrDefault().Text).FirstOrDefault();
